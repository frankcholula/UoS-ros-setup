...
place_left_poses = [[X, X, X, X],
                    [X, X, X, X]]

place_right_poses = [[X, X, X, X],
                     [X, X, X, X]]
#---------------------------------------------------
...
class PickAndPlace:
    def __init__(self, sim=False):
        ...
        if sim:
            ...
            self.step = -0.001  # For some reason "open" and "close" are swapped, so we need to invert this step
    ...
    def grip(self):  
        ...
        if sim:
            self.scene.attach_box("gripper_link", "block", touch_links=['gripper_link', 'gripper_link_sub'])
...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        option = raw_input("Attempt {}. Press enter to attempt pick and place (or enter q to quit):".format(attempts))
        if option == "q":
            break
        ...
        if sim:            
            ...
            controller.scene.add_box(box_name, box_pose, size=(box_size, box_size, box_size))
